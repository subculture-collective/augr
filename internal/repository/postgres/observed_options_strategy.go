...
func (repo *ObservedOptionsStrategyRepo) RegisterCandidate(ctx context.Context, accountID, scopeID uuid.UUID, config rules.OptionsRulesConfig, evaluationStart, evaluationEnd time.Time, sourceCommit, sourceTreeSHA256 string) (*domain.Strategy, *strategycatalog.Experiment, bool, error) {
	if repo == nil || repo.pool == nil || accountID == uuid.Nil || scopeID == uuid.Nil {
		return nil, nil, false, fmt.Errorf("postgres: observed options candidate requires database, account, and scope")
	}
	pool := repo.pool
	...
	if err != nil || !readiness.Options.Ready {
		if err != nil {
			return nil, nil, false, fmt.Errorf("postgres: observed options scope: %w", err)
		}
		return nil, nil, false, fmt.Errorf("postgres: observed options evidence: %s", readiness.Options.Reason)
	}
	if evaluationStart.Location() != time.UTC || evaluationEnd.Location() != time.UTC || !evaluationStart.Equal(evaluationStart.Truncate(time.Microsecond)) || !evaluationEnd.Equal(evaluationEnd.Truncate(time.Microsecond)) || !evaluationStart.Before(evaluationEnd) || evaluationStart.Before(readiness.EvaluationStart) || evaluationEnd.After(readiness.EvaluationEnd) || evaluationEnd.After(readiness.DecisionCutoff) {
		return nil, nil, false, fmt.Errorf("postgres: observed options evaluation interval escapes its exact scope")
	}
	...
	if _, err := catalog.RegisterStrategyFamily(ctx, family); err != nil {
		return nil, nil, false, fmt.Errorf("postgres: register observed options family: %w", err)
	}
	if _, err := catalog.RegisterStrategyVersion(ctx, version); err != nil {
		return nil, nil, false, fmt.Errorf("postgres: register observed options version: %w", err)
	}
	...
	if err := pool.QueryRow(ctx, `SELECT scope.capital_binding_id,simulation.policy_version,binding.policy_version
		...
		return nil, nil, false, fmt.Errorf("postgres: load observed options experiment parents: %w", err)
	}
	...
	if err != nil {
		return nil, nil, false, fmt.Errorf("postgres: construct observed options experiment: %w", err)
	}
	if _, err := catalog.DeclareResearchExperiment(ctx, experiment); err != nil {
		return nil, nil, false, fmt.Errorf("postgres: declare observed options experiment: %w", err)
	}
	...
	if err != nil {
		return nil, nil, false, fmt.Errorf("postgres: create observed options runtime draft: %w", err)
	}
	...
	if storedName != name || ticker != config.Underlying || market.Normalize() != domain.MarketTypeOptions || schedule != "" || status != domain.StrategyStatusInactive || !paper || active || storedVersion != version.ID() || !bytes.Equal(storedConfig, runtimeConfig) {
		return nil, nil, false, fmt.Errorf("postgres: observed options runtime draft changed on retry: %w", repository.ErrIdempotencyConflict)
	}
	var versionUses int
	if err := tx.QueryRow(ctx, `SELECT count(*) FROM strategies WHERE execution_strategy_version_id=$1`, version.ID()).Scan(&versionUses); err != nil || versionUses != 1 {
		return nil, nil, false, fmt.Errorf("postgres: observed options version resolves to %d runtime drafts", versionUses)
	}
	...
	return strategy, experiment, created, nil
}
