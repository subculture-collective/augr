...
type executionIntentCanonical struct {
	...
	MaxLossPerUnit         string               `json:"max_loss_per_unit"`
	RequiredCapitalPerUnit string               `json:"required_capital_per_unit"`
...
func NewExecutionIntent(opportunity domain.Opportunity) (*ExecutionIntent, error) {
	if opportunity.MarketType != domain.MarketTypeStock && opportunity.MarketType != domain.MarketTypeOptions {
		return nil, fmt.Errorf("portfolio execution intent supports stocks and options")
	}
	if opportunity.Ticker == "" || !opportunity.Side.IsValid() || opportunity.EntryPrice < 0 || opportunity.ProposedNotional < 0 || opportunity.ExpectedLossUSD < 0 {
		return nil, fmt.Errorf("portfolio execution intent is invalid")
	}
	legs := make([]executionIntentLeg, 0, len(opportunity.OptionLegs))
	quoteAt := ""
	if opportunity.QuoteObservedAt != nil {
		quoteAt = formatIntentTime(*opportunity.QuoteObservedAt)
	}
	if opportunity.MarketType == domain.MarketTypeOptions {
		if !validVertical(opportunity.OptionLegs) || opportunity.MaxLossPerUnit <= 0 || opportunity.RequiredCapitalUnit <= 0 || quoteAt == "" {
...
func (value *ExecutionIntent) Digest() string {
	if value == nil {
		return ""
	}
	return value.digest
...
func (value *ExecutionIntent) CanonicalBytes() []byte {
	if value == nil {
		return nil
	}
