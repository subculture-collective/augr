...
raw_snapshot="$work_dir/current.raw"
...
SELECT format('SELECT %L || E''\\t'' || count(*)::text || E''\\t'' || encode(digest(COALESCE(string_agg(to_jsonb(t)::text,E''\\n'' ORDER BY to_jsonb(t)::text),''''),''sha256''),''hex'') FROM %I.%I t',schema_name||'.'||table_name,schema_name,table_name)
...
python3 - "$raw_snapshot" "$work_dir" <<'PY'
import pathlib, sys
