...
export function PolymarketPage() {
  ...
  const [sort, setSort] = useState<'consistency_score'|'bayesian_win_rate'|'resolved_markets'|'win_rate'|'volume'|'last_active'|'trades'>('consistency_score')
  const [offset, setOffset] = useState(0)
  const accounts = usePolymarketAccounts({ tracked: trackedOnly || undefined, min_win_rate: minWinRate ? Number(minWinRate) : undefined, min_resolved: minResolved ? Number(minResolved) : undefined, sort, limit: 25, offset })
  ...
  return <div className="space-y-4" data-testid="polymarket-page"><PageHeader title="Polymarket" description="Prediction market intelligence: tracked wallets, whale trades, watched markets" />
    <Card><CardHeader><CardTitle>Summary</CardTitle></CardHeader><CardContent><div className="grid gap-3 md:grid-cols-4"><div><div className="text-xs uppercase text-muted-foreground">Displayed Wallets</div><div className="text-2xl font-semibold">{displayedTrackedCount}</div><div className="text-xs text-muted-foreground">{tracked.data?.data?.length ?? 0} tracked total</div></div><div><div className="text-xs uppercase text-muted-foreground">Total Volume</div><div className="text-2xl font-semibold">{money.format(totalVolume)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Open Polymarket Strategies</div><div className="text-2xl font-semibold">{strategiesData.length}</div></div><div><div className="text-xs uppercase text-muted-foreground">Recent Whale Signals</div><div className="text-2xl font-semibold">{signalsData.filter((s) => s.source === 'polymarket-whale').length}</div></div></div></CardContent></Card>
    ...
    <Card><CardHeader><CardTitle>Watched Markets</CardTitle></CardHeader><CardContent><div className="mb-3 flex gap-2"><Input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="market slug" /><Button onClick={doAdd}>Add</Button></div>{err ? <p className="text-sm text-destructive">{err}</p> : null}<table className="w-full text-sm"><tbody>{watchedData.map((m) => <tr key={m.slug}><td><a href={marketUrl(m.slug)} target="_blank" rel="noreferrer">{m.slug}</a></td><td><input type="checkbox" checked={m.enabled} onChange={(e) => enable.mutate({ slug: m.slug, enabled: e.target.checked })} /></td><td>{formatRelativeTime(m.added_at)}</td><td>{m.note ?? '--'}</td><td><Button size="sm" variant="outline" onClick={() => remove.mutate(m.slug)}>Remove</Button></td></tr>)}</tbody></table></CardContent></Card>
    <Card><CardHeader><CardTitle>Tracked Wallets</CardTitle></CardHeader><CardContent><div className="mb-3 flex flex-wrap gap-2"><label><input type="checkbox" checked={trackedOnly} onChange={(e) => setTrackedOnly(e.target.checked)} /> tracked-only</label><Input type="number" step="0.01" min="0" max="1" value={minWinRate} onChange={(e) => setMinWinRate(e.target.value)} placeholder="min raw win rate" /><Input type="number" step="1" min="0" value={minResolved} onChange={(e) => setMinResolved(e.target.value)} placeholder="min resolved" /><select value={sort} onChange={(e) => setSort(e.target.value as any)}><option value="consistency_score">consistency</option><option value="bayesian_win_rate">bayesian win rate</option><option value="resolved_markets">resolved markets</option><option value="win_rate">raw win rate</option><option value="volume">volume</option><option value="last_active">last active</option><option value="trades">trades</option></select><span className="text-xs text-muted-foreground">score = adjusted win rate × sample size</span></div><table className="w-full text-sm"><thead><tr><th>address</th><th>score</th><th>adj win</th><th>raw win</th><th>won/lost</th><th>volume</th><th>max_position</th><th>last_active</th><th>tracked</th><th>tags</th></tr></thead><tbody>{accountsData.map((a) => <tr key={a.address}><td><div className="flex flex-col"><Link to={`/polymarket/accounts/${a.address}`}>{shortAddress(a.address)}</Link><a className="text-xs text-muted-foreground" href={profileUrl(a.address)} target="_blank" rel="noreferrer">profile</a></div></td><td>{(a.consistency_score ?? 0).toFixed(3)}</td><td>{((a.bayesian_win_rate ?? 0) * 100).toFixed(1)}%</td><td>{(a.win_rate * 100).toFixed(1)}%</td><td>{`${a.markets_won}/${a.markets_lost} (${a.resolved_markets ?? a.markets_won + a.markets_lost})`}</td><td>{money.format(a.total_volume)}</td><td>{money.format(a.max_position)}</td><td>{formatRelativeTime(a.last_active)}</td><td><input type="checkbox" checked={a.tracked} onChange={(e) => track.mutate({ address: a.address, tracked: e.target.checked })} /></td><td>{(a.tags ?? []).map((t) => <Badge key={t}>{t}</Badge>)}</td></tr>)}</tbody></table><div className="mt-3 flex gap-2"><Button size="sm" variant="outline" disabled={offset===0} onClick={() => setOffset((o) => Math.max(0, o - 25))}>Prev</Button><Button size="sm" variant="outline" disabled={accountsData.length < 25} onClick={() => setOffset((o) => o + 25)}>Next</Button></div></CardContent></Card>
    <Card><CardHeader><CardTitle>Recent Whale Trades</CardTitle></CardHeader><CardContent><div className="mb-2 flex items-center gap-2"><span className={ws.status === 'open' ? 'size-2 rounded-full bg-emerald-400' : 'size-2 rounded-full bg-zinc-500'} /> {ws.status}</div><table className="w-full text-sm"><tbody>{(trades.data?.data ?? []).map((t) => <tr key={t.id}><td>{formatRelativeTime(t.timestamp)}</td><td><div className="flex flex-col"><Link to={`/polymarket/accounts/${t.account_address}`}>{shortAddress(t.account_address)}</Link><a className="text-xs text-muted-foreground" href={profileUrl(t.account_address)} target="_blank" rel="noreferrer">profile</a></div></td><td><div className="flex flex-col"><a href={marketUrl(t.market_slug)} target="_blank" rel="noreferrer">{t.market_slug}</a><a className="text-xs text-muted-foreground" href={eventUrl(t.market_slug)} target="_blank" rel="noreferrer">event</a></div></td><td><Badge variant={String(t.side).toUpperCase()==='YES' ? 'success' : 'destructive'}>{t.side}</Badge></td><td>{money2.format(t.size_usdc)}</td><td>{t.price.toFixed(3)}</td></tr>)}</tbody></table></CardContent></Card>
