...
export function normalizeStatus(value: string | undefined | null): AppStatus {
  ...
  if (new RegExp(`^(?:skipped|blocked) after ${duration}$`).test(normalized)) return 'warning'
  ...
  if (['warning', 'degraded', 'disconnected', 'tripped', 'skipped', 'blocked'].includes(normalized)) return 'warning'
